...
/*!
 @brief 推流截图回调block
 */
typedef void(^ChuangPublisherTakeSnapshotCallback)(NSString * _Nullable streamId, int errorCode, UIImage * _Nullable image);
 ...
 */
typedef NS_ENUM(NSUInteger, ChuangVideoCaptureRotation) {
    /*!
       @brief 竖屏； Home键在下
    */
    ChuangVideoCaptureRotationPortrait = 0,
    
    /*!
       @brief 横屏； Home键在右
    */
    ChuangVideoCaptureRotationRight = 1,
...
/*!
 @brief 日志过滤器
 */
typedef NS_ENUM(NSUInteger, ChuangLogFilter) {
    
    /** 不输出日志信息，上线前请关闭日志输出 */
    ChuangLogFilterOff       = 0,
    
    /** 输出所有 API 日志信息。如果你想获取最完整的日志，可以将日志级别设为该等级。 */
    ChuangLogFilterDebug     = 1,
    
    /** 输出 CRITICAL、ERROR、WARNING 和 INFO 级别的日志信息。我们推荐你将日志级别设为该等级 */
    ChuangLogFilterInfo      = 2,
    
    /**输出 CRITICAL、ERROR 和 WARNING 级别的日志信息 */
    ChuangLogFilterWarning   = 3,
    
    /** 输出 CRITICAL 和 ERROR 级别的日志信息 */
    ChuangLogFilterError     = 4,
    
    /** 输出 CRITICAL 级别的日志信息 */
    ChuangLogFilterCritical  = 5,
};
