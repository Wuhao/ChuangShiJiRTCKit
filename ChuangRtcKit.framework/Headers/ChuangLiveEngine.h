...
/*!
 @brief 混音数据回调
 @param mxingData 数据，详见ChuangAudioMxingData
 */
- (void)onAudioMixingCopyData:(ChuangAudioMxingData *)mxingData;
...
/*!
 @brief 自定义编码流控信息回调（选择自定义采集已编码数据的前提才会返回）
...
/*!
 ...
 @param buffer 封装为CVPixelBuffer的视频帧数据
 @param streamId 推流的流Id
 */
- (void)onCapturedVideoFrameCVPixelBuffer:(CVPixelBufferRef)buffer streamId:(NSString *)streamId;
...
*/
+ (instancetype)initEngine:(NSString *)appId andAppKey:(NSString *)appKey delegate:(id <ChuangLiveEngineDelegate>)delegate;
...
@brief 初始化本地视频视图
 ...
 该方法初始化本地视图并设置本地用户视频显示信息，只影响本地用户看到的视频画面，不影响本地发布视频。调用该方法绑定本地视频流的显示视窗(view)，并设置本地用户视图的渲染模式和镜像模式。在 App开发中 开发中，通常在初始化后调用该方法进行本地视频设置，然后再登录房间。登出房间后，绑定仍然有效，如果需要解除绑定，可以指定空 (nil) view 调用本方法。
 该方法在登录房间前后都能调用。
...
/*!
@brief 获取推流视频截图
@param imageCallback 截图回调  详情见ChuangPublisherTakeSnapshotCallback
@return 调用结果 0：成功，非0：失败
*/
- (int)takePublishStreamSnapshot:(ChuangPublisherTakeSnapshotCallback)imageCallback;
...
/*!
 ...
 @return 调用结果  0：成功，非0：失败
 */
- (int)setAudioMixingHandler:(id<ChuangAudioMixingDelegate>)delegate;
...
/*!
 ...
 @return 调用结果  0：成功，非0：失败
 */
- (int)enableCustomVideoRender:(BOOL)enable config:(ChuangCustomVideoRenderConfig *)config;
/*!
 @brief 设置自定义渲染视频回调
...
@param roomId 房间ID
@param streamId 流ID
@param newRenderMode 新的渲染模式。详见ChuangVideoRenderMode
*/
- (void)updateRemoteStreamRenderModeWithRoomId:(NSString *)roomId andStreamId:(NSString *)streamId andRenderMode:(ChuangVideoRenderMode)newRenderMode;

/*!
@brief 更新房间内播流渲染模式
@param streamId 流ID
@param newRenderMode 新的渲染模式，详见：ChuangVideoRenderMode
*/
- (void)updateRemoteStreamRenderModeWithStreamId:(NSString*)streamId andRenderMode:(ChuangVideoRenderMode)newRenderMode;


/*!
@brief 更新本地视频预览显示模式
@param videoRenderMode 视频显示模式（是否拉伸等），详情见ChuangVideoRenderMode
*/
- (void)updateLocalVideoRenderMode:(ChuangVideoRenderMode)videoRenderMode;


/*!
@brief 设置本地预览镜像模式
@param mirrorMode  详情参照ChuangVideoMirrorMode，不设置则采用SDK默认，
*/
- (void)setLocalRenderMirrorMode:(ChuangVideoMirrorMode)mirrorMode;


/*!
@brief 设置摄像头采集方向，默认是竖屏采集，不调用会默认竖屏
@param captureRotation   详情参照ChuangVideoCaptureRotation，内部默认竖屏采集，当需要指定横屏采集的时候，如果需要横屏采集需要在开始预览和推流前设置
*/
- (void)setDeviceCaptureDirection:(ChuangVideoCaptureRotation)captureRotation;
